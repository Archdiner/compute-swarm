...
LORA_FINETUNE_TEMPLATE = JobTemplate(
    name="lora_finetune",
    description="Fine-tune a model using LoRA with Gradient Accumulation for consumer GPUs",
...
Generated by ComputeSwarm
Optimizer for Consumer GPUs (Swarm Edition)
"""
...
if device != "cuda":
    print("WARNING: LoRA fine-tuning is extremely slow without GPU")
...
# Enable Gradient Checkpointing (Critical for VRAM saving)
...
try:
    ...
    # Heuristic: use 'train' split, or first available split
    split_name = "train" if "train" in dataset else list(dataset.keys())[0]
    dataset = dataset[split_name]
    
    # Cap dataset for demo purposes if it's huge, but allow real training
    if len(dataset) > 10000:
        print(f"Dataset too large ({len(dataset)}), using 10k sample subset for efficiency")
    ...
    dataset = Dataset.from_dict({"text": ["This is a training example"] * 100})

def tokenize_function(examples):
    # Simple formatting - assumes 'text' column or similar
